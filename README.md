...
Instead of basic queries, this portfolio applies **Window Functions, Common Table Expressions (CTEs), and Conditional Aggregations** to uncover portfolio vulnerabilities, evaluate collateral coverage, and diagnose critical financial trends.
...
To establish a robust relational framework for credit risk analysis, the database design enforces strict referential integrity and optimized mapping across the retail lending portfolio.
...
### 1. Credit Score Drift & Trajectory Modeling (`LAG()` & `CASE WHEN`)
...
### 2. Under-Collateralized Asset Exposure (Loan-to-Value Risk)
...
### 3. Portfolio Concentration & Liquidity Forecasting (`SUM` & `ROW_NUMBER`)
...
* **The Catch:** Isolated the single highest-exposure loan contract deployed within each monthly vintage, identifying seasonal concentration spikes and uncovering testing placeholder patterns inside secondary guarantor systems.
...
> ⚠️ **Important:** All raw source datasets required for this project are fully posted inside the `Datasets/` directory. To reproduce the analytical findings, you must explicitly import these files into your local SQL engine before running the core analytical scripts.

Follow this complete step-by-step sequence to construct the target environment and ingest the tables properly:
...
Load the posted `.csv` source data files from your local folder into your schema in this exact structural order:
1. `BORROWERS.csv`
